...
describe('ProfileApiService', () => {
  ...
  it('reads and updates only the self-service profile contract', async () => {
    ...
    await service.updateMyProfile({firstName: 'Lisa', lastName: 'Chen', emailNotifications: false});
    expect(client.get).toHaveBeenCalledWith('/v2/me/profile');
    expect(client.patch).toHaveBeenCalledWith('/v2/me/profile', {firstName: 'Lisa', lastName: 'Chen', emailNotifications: false}, expect.objectContaining({headers: expect.any(Object)}));
  });
