...
describe('CourseMembershipPanel', () => {
  beforeEach(() => {
    mocks.browseCourses.mockResolvedValue(success({
      items: [{
        id: 31,
        courseId: 31,
        tenantId: 1,
        ...
        state: 'Active',
        instructorId: 7,
        primaryInstructor: {userId: 7, instructorFirstName: 'Professor', instructorMiddleName: null, instructorLastName: 'Ada', email: 'ada@example.edu'},
      }],
      page: 0,
      size: 100,
      total: 1,
    }));
    mocks.listCourseMembers.mockResolvedValue(success({
      items: [
        {
          id: 1,
          courseId: 31,
          userId: 7,
          userFirstName: 'Professor',
          userMiddleName: null,
          userLastName: 'Ada',
    ...
    mocks.enrolStudents.mockResolvedValue(success({
      requestedCount: 1,
      successCount: 1,
      failureCount: 0,
      items: [{userId: 490, status: 'SUCCESS', errorType: null, message: null, member: null}],
    }));
  ...
  it('anchors enrollment to a course and its primary instructor', async () => {
    ...
    expect(await screen.findByRole('option', {name: 'CSCI-101 — Foundations of Computing · Professor Ada'})).toBeInTheDocument();
  ...
  it('requires review before changing a student into a course-scoped TA', async () => {
    ...
    expect(screen.getByText(/Existing student submissions in this course will be frozen/)).toBeInTheDocument();
    expect(mocks.promoteToTa).not.toHaveBeenCalled();

    await user.click(screen.getByRole('button', {name: 'Confirm TA assignment'}));

    await waitFor(() => expect(mocks.promoteToTa).toHaveBeenCalledWith(31, 485));
    expect(await screen.findByText('TA assigned for the selected course.')).toBeInTheDocument();
  });
});
