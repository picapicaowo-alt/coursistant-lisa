    ...
    (async () => {
      try {
        const response = await axios.post(
          `${API_DOMAIN}/thirdParty/google/continue?authorizationCode=${code}`
        );

        let redirectUrl = '';
        console.log("resp from Google handler:", response);
        ...
        if (response.data.data && response.data.code === "4021") {
          ...
          localStorage.setItem("usernotexistuserEmailGoogle", email);
          localStorage.setItem("usernotexistuserNameGoogle", name);
        ...
        console.error("Error calling Google continue API:", err);
        setMessage('An error occurred. Redirecting to login.');
...
export default GoogleRerouteHandler;
