    ...
    (async () => {
      try {
        const response = await axios.post(
          `${API_DOMAIN}/thirdParty/facebook/continue?authorizationCode=${code}`
        );

        let redirectUrl = '';
        console.log("resp from Facebook handler:", response);
        ...
        if (response.data.data && response.data.code === "4021") {
          ...
          localStorage.setItem("usernotexistuserEmailFacebook", email);
          localStorage.setItem("usernotexistuserNameFacebook", name);
        ...
        console.error("Error calling Facebook continue API:", err);
        setMessage('An error occurred. Redirecting to login.');
    ...
    >
      ...
      <div
        style={{
          ...
          borderTop: '14px solid #1877F2', // Facebook blue
          borderRadius: '50%',
          width: '60px',
          height: '60px',
...
export default FacebookReRouteHandler;
