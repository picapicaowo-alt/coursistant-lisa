...
const LinkedinReRouteHandler = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const url = `${API_DOMAIN}/thirdParty/linkedin/continue?authorizationCode=${code}`;
        console.log("Calling LinkedIn API:", url);
        const response = await axios.post(url);
        let redirectUrl = '';
        console.log("LinkedIn handler response:", response);

        if (response.data.data && response.data.code === "4021") {
          const { email, name } = response.data.data;
          localStorage.setItem("socialProvider", "linkedin");
          localStorage.setItem("usernotexistuserEmailLinkedIn", email);
          localStorage.setItem("usernotexistuserNameLinkedIn", name);
        ...
        console.error("Error calling LinkedIn continue API:", err);
        setMessage('An error occurred. Redirecting to login.');
...
export default LinkedinReRouteHandler;
