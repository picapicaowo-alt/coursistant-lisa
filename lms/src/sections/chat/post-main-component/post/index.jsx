...
const Post = ({ onSelectPost }) => {
    ...
    const postTemplate = 
    {
        ...
        time: "12:14 PM",
        postType: "Question",
        body: "In this course, we'll be teaching the concepts of the JavaScript programming language and the cool ...",
        instructor: {
        name: "Sylvia Reyes",
        role: "Instructor",
    ...
    return (
        <div className="flex items-center justify-center flex-col">
                    ...
                    {postContentCategories.map((item, index) => (
                        <div className="flex items-center gap-2" key={index}>
                            <span className={`text-[1.1rem] font-medium text-[rgba(45,55,72,1)] cursor-pointer border-b-2
                                ${selectedPostContentCategory === item ? "border-[rgba(86,111,232,1)]" : "border-transparent"}`} onClick={() => setSelectedPostContentCategory(item)}>{item}</span>
                        </div>
                    ))}
            </div>
            {/* Post Categories */}
            <div className="mt-5 flex flex-row justify-between w-full px-10">
                {/* Sort Dropdown */}
                <div ref={sortDropdown.ref} className="relative cursor-pointer flex items-center gap-2 border-1 border-[rgba(203,213,224,1)] hover:bg-[rgba(226,232,240,1)] rounded-lg px-4 py-1">
                        ...
                        {sortDropdown.isOpen && (
                            <div className="absolute mt-43 left-0 w-40 rounded-md shadow-lg bg-white ring-1 ring-black/5 z-10">
                                {sortOptions.map((item, index) => (
                                    <div key={index} className="cursor-pointer px-4 py-2 hover:bg-gray-100 text-primary-color" onClick={() => handleSortSelect(item)}>{item}</div>
                                ))}
                            </div>
                        )}
                </div>
                ...
                <div className="relative cursor-pointer flex items-center gap-2 border-1 border-[rgba(203,213,224,1)] hover:bg-[rgba(226,232,240,1)] rounded-lg px-4 py-1">
